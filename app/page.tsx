...
export default function Component() {
  ...
  const pollingInterval = 30_000; // 0.5 minutes

  useEffect(() => {
    let isMounted = true; // Flag to handle component mount state

    const fetchData = async () => {
      if (!isMounted) return; // Avoid state updates if the component is not mounted
      setIsLoading(true);
      try {
        ...
        const newMetricsData = [
          ...
          },
          {
            ...
            percentageOfTarget: roundToThree(
              Math.min(
                (distraction_count / parseFloat(targets.distraction_count)) *
                  100,
                100
              )
            ),
